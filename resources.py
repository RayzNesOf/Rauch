class ResourceManager:
    ...
    def calculate_daily_production(self, minister_efficiency):
        """Расчет ежедневного производства"""
        food_production = (self.food_factory * 1.0 + self.bakery * 0.5) * minister_efficiency.get('agriculture', 1.0)
        ammo_production = self.underground_factory * 1.2 * minister_efficiency.get('industry', 1.0)
        fuel_production = self.boiler_house * 0.3 + (self.power_plant * 0.1) * minister_efficiency.get('resources', 1.0)

        return food_production, ammo_production, fuel_production

    def calculate_daily_consumption(self, population, soldiers, battles_count, patrols, motorized_divisions):
        """Расчет ежедневного потребления"""
        food_consumption = population * 0.02 + soldiers * 0.05
        ammo_consumption = battles_count * 150 + patrols * 50
        fuel_consumption = motorized_divisions * 100

        self.food_consumption = food_consumption
        self.ammo_consumption = ammo_consumption
        self.fuel_consumption = fuel_consumption

        return food_consumption, ammo_consumption, fuel_consumption

    def update_resources(self, production, consumption):
        """Обновление ресурсов после производства/потребления"""
        food_prod, ammo_prod, fuel_prod = production
        food_cons, ammo_cons, fuel_cons = consumption
